  ...
  local function pedals_add(use_diagram)
    ...
    for cat in eachbackwards(categorydefs) do
      if cat:CreateName().LuaString == "Technique Text" and diagrams == 0 then
        diagrams = cat.ID
        diagrams_cat = cat
        use_tech = true -- luacheck: ignore use_tech
        if use_diagram then
...
-------
      ...
      if harp_error then
        ...
        ui:AlertYesNo("There seems to be a problem with your harp diagram. \n Would you like to try again?", nil)
      end -- error
      if is_dialog_assigned then
        ui:AlertInfo("There is already a harp diagram assigned to this region.", nil)
      end
...
----
      ...
      if scale == "whole tone" or scale == "major pentatonic" or scale == "minor pentatonic" then
        use_chord = false -- luacheck: ignore use_chord
      end -- temporary change for exotic scales!
...
--
      ...
      str.LuaString = "Harp Pedal Wizard v"..finaleplugin.Version
      dialog:SetTitle(str)
-----
...
--      local accidentals = {"♭", "♮", "♯"} -- for later implementation
        ...
        --
        row_y = row_y + 32
        dialog:CreateHorizontalLine(0, row_y - 6, 320)
        ...
        --
        row_y = row_y + 26
        dialog:CreateHorizontalLine(0, row_y-8, 320)
        --
        str.LuaString = "D"
        ...
        if ui:IsOnMac() then
          ...
          dialog:CreateHorizontalLine(col_x, nat_y + 6, col_width * 7)
        end
        --
        ...
        if ui:IsOnMac() then
          ...
          dialog:CreateVerticalLine(col_x + (col * col_width) - 1, flat_y, row_h * 3)
        end
